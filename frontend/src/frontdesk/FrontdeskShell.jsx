...
import { Anchor, Bell, BedDouble, BriefcaseBusiness, CalendarDays, ChevronRight, ClipboardList, Home, Layers, LayoutDashboard, Loader2, LogOut, MapPin, ReceiptText } from 'lucide-react';
...
export default function FrontdeskShell() {
  ...
  return (
    <div className="flex min-h-screen bg-app-bg text-app-text">
      <aside className="flex h-screen w-72 shrink-0 flex-col border-r border-[#d7b56d]/20 bg-[#172326] text-white shadow-2xl shadow-black/15">
        ...
        <nav className="flex flex-1 flex-col gap-2 px-4 py-8">
          {navItems.map((item) => {
            ...
            return (
              <Link key={item.path} to={item.path} className={`flex items-center gap-4 rounded-2xl border-2 px-4 py-3.5 text-sm font-black uppercase tracking-widest transition-all ${active ? 'border-[#d7b56d]/30 bg-[#d7b56d]/15 text-[#f1d58a]' : 'border-transparent text-white/62 hover:border-white/10 hover:bg-white/8 hover:text-white'}`}>
        ...
        <header className="sticky top-0 z-10 border-b border-[#d7b56d]/20 bg-[#172326]/95 text-white backdrop-blur-xl">
          <div className="px-8 py-5">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-2xl font-black text-white">{routeLabels[location.pathname] || 'Frontdesk'}</h1>
                <p className="text-sm text-white/58">Rooms, reservations, in-house guests, folios, and requests</p>
              </div>
            </div>
          </div>
        </header>
